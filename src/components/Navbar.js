...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // Scrollspy effect
    const sections = ["home", "about", "Featured Work", "contact"];
    const observer = new IntersectionObserver(
      ...
      {
        root: null,
        rootMargin: "-50% 0px -50% 0px", // Trigger when section is near middle of viewport
        threshold: 0,
      }
    );
          ...
          {["home", "about", "Featured Work", "contact"].map((link) => (
            <a
              ...
              className={`relative text-lg px-4 py-2 font-medium text-[#004E50] transition-all duration-300
                ...
                ${active === link ? "font-semibold after:w-full" : ""}`}
            >
          ...
          {["home", "about", "Featured Work", "contact"].map((link) => (
            <a
              ...
              className={`relative text-lg px-6 py-2 font-medium transition-all duration-300
                ...
                hover:after:w-full hover:font-semibold
                ${active === link ? "font-bold after:w-full" : ""}`}
            >
              {link.charAt(0).toUpperCase() + link.slice(1)}
            </a>
          ))}
